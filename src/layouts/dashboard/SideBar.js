...
const Sidebar = () => {
    ...
    return (
        <Box sx={{ display: "flex", height: "100vh", width: "100%", gap: 1 }}>
            {/* Sidebar */}
            <Box sx={{ backgroundColor: theme.palette.background.paper, boxShadow: "0px 0px 2px rgba(0,0,0,0.25)", height: "100%", width: 70, py: 2 }}>
                <Stack direction="column" alignItems="center" justifyContent="space-between" sx={{ height: "100%" }} spacing={3}>
                    <Stack alignItems="center" spacing={4} sx={{ height: "100%" }}>
                        <Box sx={{ backgroundColor: theme.palette.primary.main, height: 54, width: 54, borderRadius: 1.5, display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <img src={Logo} alt="chatAt Logo" width={32} height={32} />
                        </Box>

                        <Stack spacing={3} alignItems="center">
                            {Nav_Buttons.map((item) => (
                                <Box key={item.index} sx={{ backgroundColor: selected === item.index ? theme.palette.primary.main : "transparent", borderRadius: 1.5 }}>
                                    <IconButton
                                        onClick={() => {
                                            setSelected(item.index);
                                            if (item.path) navigate(item.path);
                                        }}
                                        sx={{ color: selected === item.index ? theme.palette.primary.contrastText : theme.palette.text.secondary }}
                                    >
                                        {item.icon}
                                    </IconButton>
                                </Box>
                            ...
                            <Box sx={{ backgroundColor: selected === 3 ? theme.palette.primary.main : "transparent", borderRadius: 1.5 }}>
                                <IconButton
                                    onClick={() => {
                                        setSelected(3);
                                        navigate("/Setting");
                                    }}
                                    sx={{ color: selected === 3 ? "#fff" : theme.palette.text.primary }}
                                >
                                    <Gear />
                                </IconButton>
                            </Box>
                    ...
                    <Stack spacing={2} alignItems="center">
                        ...
                        <Avatar src={"/avatar.png"} onClick={handleClick} sx={{ cursor: "pointer" }} />
                        <Menu
                            anchorEl={anchorEl}
                            open={open}
                            onClose={handleClose}
